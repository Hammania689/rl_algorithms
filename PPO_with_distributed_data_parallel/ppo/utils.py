...
def get_rollouts(env: Pendulum, 
    ...
    # Update \pi_theta_old 
    actor.module.set_distr_old(rollout_states.to(device))
    ...
    with torch.no_grad():
        ...
        for step in range(num_steps):
            ...
            if hardcode_actor:
                ...
                if isinstance(actor, torch.nn.parallel.DistributedDataParallel):
                    a = actor.module.distr_type(mu, std).sample().cpu().numpy()
                else:
                    a = actor.distr_type(mu, std).sample().cpu().numpy()
...
def final_plots(critic_losses, total_rewards, step, cur_step, dir_name: str = './results'):
    # save plot of losses curve
    fig, ax = plt.subplots(1, 1)
    ax.semilogy(step, critic_losses, label='mean critic loss')
    ax.grid()
    ax.set_xlim(0, step[-1])
    ax.set_xlabel('total number of simulation steps')
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(dir_name, f'criticlosscurve_{cur_step}.png'))
    plt.close()
    ...
    fig, ax = plt.subplots(1, 1)
    size = 50
    size = size if len(total_rewards) > size else 1
    payoff = pd.Series(total_rewards)
    payoff_mean = payoff.rolling(size).mean()
    payoff_std = payoff.rolling(size).std()
    ax.plot(step, total_rewards, '.', markersize=1, alpha=0.1, color='C0')
    ax.plot(step, payoff_mean, '-', linewidth=2, label='average reward per step', alpha=1.0, color='C1')
    ax.fill_between(step, payoff_mean - payoff_std, payoff_mean + payoff_std, alpha=0.6, color='C1')
    ax.grid()
    ax.set_xlim(0, step[-1])
    ax.set_xlabel('total number of simulation steps')
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(dir_name, f'learningcurve_{cur_step}.png'))
    plt.close()
...
def plot_gif(env, actor, critic, device, step, hardcode_actor=False, dir_name: str = './results'):
